...
function buildStationStatus(available: number, total: number) {
  if (available <= 0) {
    return {
      statusKey: 'queue' as StationItem['statusKey'],
      statusText: 'Busy',
    }
  }
  ...
  if (total > 0 && available / total <= 0.3) {
    return {
      statusKey: 'busy' as StationItem['statusKey'],
      statusText: 'Partially busy',
    }
  }

  return {
    statusKey: 'idle' as StationItem['statusKey'],
...
function mapOrder(value: unknown): OrderDetailItem {
  ...
  const orderStatusRaw = String(pickValue(source, ['status', 'order_status', 'orderStatus'], 'completed'))
  ...
  const durationMinutes =
    toNumber(pickValue(source, ['duration_minutes', 'duration', 'durationMinutes'], 0)) ||
    (endTime !== '--' ? diffMinutes(startTime, endTime) : 0)
  const energyValue = toNumber(
    pickValue(source, ['total_power', 'power', 'energy', 'charged_kwh', 'total_energy'], 0)
  )
  const electricityFeeValue = toNumber(
    pickValue(source, ['electricity_fee', 'power_fee', 'energy_fee'], 0)
  )
  ...
  const totalAmountValue = toNumber(
    pickValue(source, ['total_amount', 'amount', 'pay_amount', 'payable_amount'], 0)
  )
  const paymentStatus = String(
    pickValue(source, ['payment_status', 'paymentStatus'], status === 'charging' ? 'pending' : 'paid')
  )
  ...
  return {
    ...
    pileNo: String(pickValue(source, ['pile_no', 'pileNo', 'sn_code', 'snCode'], '--')),
...
function mapWalletRecord(value: unknown): WalletRecord {
  ...
  return {
    ...
    title: String(pickValue(source, ['title', 'station_name', 'stationName', 'remark'], 'Wallet change')),
...
function mapInvoiceRecord(value: unknown): InvoiceRecord {
  const source = toRecord(value)
  const status = String(pickValue(source, ['status', 'invoice_status'], 'pending')).toLowerCase()
  ...
  return {
    ...
    status: normalizedStatus as InvoiceRecord['status'],
    statusText:
...
export function buildChargingSessionFromOrder(order: OrderDetailItem): ChargingSession {
  return {
    ...
    durationMinutes: order.endTime !== '--' ? diffMinutes(order.startTime, order.endTime) : 0,
    ...
    priceNote: 'Realtime demo values are shown locally. Backend order remains source of truth.',
  }
}
